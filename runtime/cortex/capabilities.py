...
CAPABILITIES: list[tuple[str, str]] = [
    ...
    ("Sales-loop tracking — LIVE",
     ...
     "Meetings on a deal land on its timeline with our action items tracked. An email drafted with "
     "draft_email is LINKED to its deal (pass deal_id, or the recipient's one active deal is used), and when a "
     "quotation goes out the deal's value becomes that quotation's total before VAT and the timeline says "
     "which version went for how much. deal_timeline opens one "
    ...
    ("Agency AI production — trade rates, guide quotation, standard deck",
     ...
     "the direct-client AI rates: single video up to 30 seconds, batches of 5, 10 and 15, AI voice-over per video "
     "(no human voice-over: we pitch AI), two social formats included, extra format, two revision rounds per batch. "
     "agency_rates_deck returns the standard Agency AI Production Rates deck (the prices are IN the deck: send it on "
     "its own, never a quotation with it unless the agency confirms a scope), or builds a version with the agency's "
     "name on it; offer it whenever an agency asks what AI video costs. A worked 'Agency Guide Quotation' sits in the "
     "terms folder as a team reference."),
    ...
    ("Quotations that match their proposal",
     ...
     "lists its rate-card `components`: CODE prices every line from the card, you never type a price. "
     "A typed price survives only if Rashad wrote that figure or it is a card rate. When a proposal "
     "already STATES the figures, quote them exactly: a quotation that contradicts the deck the client "
     "is reading is the failure to avoid. `total` = a number Rashad gives: component lines move within "
     "the company's band (Sensa 20%) to reach it, outside the band nothing is built and he is told what "
     "it would take. Presets carry their own payment schedule (AI production 70/30; a shoot commits a "
     "studio and crew to a date, so 50/25/25)."),
